...
${K8S_ROOT:-/opt/k8s}/bin/kubectl create clusterrolebinding \
  ...
  --user kubernetes \
  --kubeconfig ${K8S_ROOT:-/opt/k8s}/conf/kubectl.kubeconfig
...
${K8S_ROOT:-/opt/k8s}/bin/kubectl apply \
  -f ${K8S_ROOT:-/opt/k8s}/conf/csr-crb.yaml \
  --kubeconfig ${K8S_ROOT:-/opt/k8s}/conf/kubectl.kubeconfig

if [ -f ${K8S_ROOT:-/opt/k8s}/conf/kubelet-bootstrap.kubeconfig ];then
  exit 0
fi
...
export BOOTSTRAP_TOKEN=$(${K8S_ROOT:-/opt/k8s}/bin/kubeadm token create \
    ...
    ${K8S_ROOT:-/opt/k8s}/bin/kubectl config set-cluster kubernetes \
      ...
      --kubeconfig=${K8S_ROOT:-/opt/k8s}/conf/kubelet-bootstrap.kubeconfig
    ...
    # 设置默认上下文
    ${K8S_ROOT:-/opt/k8s}/bin/kubectl config use-context default --kubeconfig=${K8S_ROOT:-/opt/k8s}/conf/kubelet-bootstrap.kubeconfig

    chmod 644 ${K8S_ROOT:-/opt/k8s}/conf/kubelet-bootstrap.kubeconfig
