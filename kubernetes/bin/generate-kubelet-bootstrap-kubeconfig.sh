...
export BOOTSTRAP_TOKEN=$(/opt/bin/k8s/kubeadm token create \
    ...
    /opt/bin/k8s/kubectl config set-cluster kubernetes \
      ...
      --kubeconfig=${K8S_CONF_PATH:-/opt/bin/k8s/conf}/kubelet-bootstrap.kubeconfig

    # 设置客户端认证参数
    /opt/bin/k8s/kubectl config set-credentials kubelet-bootstrap \
      --token=${BOOTSTRAP_TOKEN} \
      --kubeconfig=${K8S_CONF_PATH:-/opt/bin/k8s/conf}/kubelet-bootstrap.kubeconfig

    # 设置上下文参数
    /opt/bin/k8s/kubectl config set-context default \
      --cluster=kubernetes \
      --user=kubelet-bootstrap \
      --kubeconfig=${K8S_CONF_PATH:-/opt/bin/k8s/conf}/kubelet-bootstrap.kubeconfig

    # 设置默认上下文
    /opt/bin/k8s/kubectl config use-context default --kubeconfig=${K8S_CONF_PATH:-/opt/bin/k8s/conf}/kubelet-bootstrap.kubeconfig

    chmod 644 ${K8S_CONF_PATH:-/opt/bin/k8s/conf}/kubelet-bootstrap.kubeconfig
