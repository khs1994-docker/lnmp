...
# 1.前端构建
FROM node:${NODE_VERSION:-12.3.1}-alpine as frontend
...
# 2.安装 composer 依赖
FROM ${DOCKER_HUB_USERNAME:-khs1994}/php:7.3.6-composer-alpine as composer
...
# 3.将项目打入 PHP 镜像
# $ docker build -t khs1994/php:7.3.6-pro-GIT_TAG-alpine --target=php .
