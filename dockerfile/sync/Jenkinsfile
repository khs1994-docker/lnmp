pipeline {
  ...
  stages {
    ...
    stage('同步镜像') {
      steps {
        sh "curl -L -O https://gitee.com/khs1994-docker/lnmp/raw/24.11/dockerfile/sync/docker-image-sync.json"
        ...
        docker run -i --rm \
            ...
            -e CI=true \
            -e CONFIG_URL=https://gitee.com/khs1994-docker/lnmp/raw/24.11/dockerfile/sync/docker-image-sync.json \
