...
RUN --mount=type=bind,from=php:alpine,source=/usr/local/bin,target=/opt/bin,rw \
  ...
  && if [ -d php ];then \
       cd php; \
       git reset --hard origin/PHP-7.4; \
       git pull --depth=1; \
     else \
       git clone --depth=1 -b PHP-7.4 ${PHP_GIT} php; \
     fi \
  \
  && wget https://github.com/tideways/php-xhprof-extension/archive/master.zip \
  && unzip master.zip \
  && mv php-xhprof-extension-master php/ext/tideways_xhprof \
  && rm -rf master.zip \
  \
...
# https://manpages.debian.org/stretch/libpcre3-dev/pcrejit.3.en.html#AVAILABILITY_OF_JIT_SUPPORT
    ...
    $(test `uname -m` = 'x86_64' && echo '--enable-tideways_xhprof=shared') \
		\
