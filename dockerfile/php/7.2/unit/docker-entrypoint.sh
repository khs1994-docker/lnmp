...
# copy from https://github.com/nginx/unit/blob/master/pkg/docker/docker-entrypoint.sh
...
if [ "$1" = "unitd" ]; then
    if /usr/bin/find "/usr/local/nginx-unit/state/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            ...
            /usr/sbin/unitd --user root --group root --log /var/log/nginx-unit/nginx-unit.log --control /usr/local/nginx-unit/control.unit.sock
            ...
            /usr/bin/curl -s -X GET --unix-socket /usr/local/nginx-unit/control.unit.sock http://localhost/
