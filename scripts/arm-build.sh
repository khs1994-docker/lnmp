...
manifest  [7.3.11] [TYPE:fpm | composer | unit | swoole | supervisord ]
build     [7.3.11] [ arm32v7 | arm64v8 ] [7.3/alpine] [TYPE: fpm | composer | ... ]
...
if [ "$1" = 'manifest' ];then
  manifest ${2:-7.3.11} ${3:-fpm}
  exit
fi

if [ "$1" = 'build' ];then
  build ${2:-7.3.11} ${3:-arm32v7} ${4:-7.3/alpine} ${5:-fpm}
  exit
fi

# build 7.2.24 arm32v7 7.2/alpine fpm
# build 7.2.24 arm64v8 7.2/alpine fpm

build 7.3.11 arm32v7 7.3/alpine fpm
build 7.3.11 arm32v7 7.3/composer composer

build 7.3.11 arm64v8 7.3/alpine fpm
build 7.3.11 arm64v8 7.3/composer composer

manifest 7.3.11 fpm
manifest 7.3.11 composer
