...
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a development       -d     'Use LNMP in Development(Support x86_64 arm32v7 arm64v8)'
...
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a init              -d      ' '
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a commit            -d      ' '
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a test              -d      ' '
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a dockerfile-update -d      'Update Dockerfile By Script'
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a debug             -d      'Debug environment'
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a cn-mirror         -d      'Push master branch to CN mirror'
complete -c lnmp-docker.sh -n "__fish_use_subcommand" -f -a compose           -d      'Install docker-compose By curl from github'
